defmodule IpProjectAkseleleWeb.PetControllerTest do
  use IpProjectAkseleleWeb.ConnCase

  alias IpProjectAkselele.PetContext

  @create_attrs %{cat_or_dog: "some cat_or_dog", name: "some name"}
  @update_attrs %{cat_or_dog: "some updated cat_or_dog", name: "some updated name"}
  @invalid_attrs %{cat_or_dog: nil, name: nil}

  def fixture(:pet) do
    {:ok, pet} = PetContext.create_pet(@create_attrs)
    pet
  end
  ...
  describe "update pet" do
    ...
    test "redirects when data is valid", %{conn: conn, pet: pet} do
      ...
      assert html_response(conn, 200) =~ "some updated cat_or_dog"
    end
